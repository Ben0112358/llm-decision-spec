...
def test_and_idempotent(context):
    A = DummyOperator([event for event in context.events if context.key(event) in {1, 2}])
...
def test_and_commutative(context):
    A = DummyOperator([event for event in context.events if context.key(event) in {1, 2}])
    B = DummyOperator([event for event in context.events if context.key(event) in {2, 3}])

    r1 = And(A, B).evaluate(context)
    r2 = And(B, A).evaluate(context)

    assert {context.key(event) for event in r1.data} == {context.key(event) for event in r2.data}
...
def test_and_associative(context):
    A = DummyOperator([event for event in context.events if context.key(event) in {1, 2}])
    B = DummyOperator([event for event in context.events if context.key(event) in {2, 3}])
    C = DummyOperator([event for event in context.events if context.key(event) in {3}])
